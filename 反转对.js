...
function getParis(nums){
    ...
    for(var i = 1; i < len; i++){
        ...
        high = i - 1
        if(cur <= nums[0]){
            sum = insertAndPairs(nums,0,i - 1,cur,sum)
            nums[0] = cur
        }else if(cur >= nums[i - 1]){
            continue
        }else{
            var index = midSearch(nums,low,high,cur)
            sum = insertAndPairs(nums,index,i - 1,cur,sum)
        }
    }
    return sum
}

/*function midSearch(nums,low,high,value){
    var mid = parseInt((low + high)/2)
    if(value > nums[mid]){
        mid++
        if(mid < high){
             return midSearch(nums,mid,high,value)
        }else{
            return mid
        }
        ...
        mid--
        if(mid > low){
             return midSearch(nums,low,mid,value)
        }else{
            return mid
        }
    }
...
function midSearch(nums,low,high,value){
    ...
    if(nums[mid] > value){
        return midSearch(nums,low,mid - 1,value)
    }
    if(nums[mid] < value){
        return midSearch(nums,mid + 1,high,value)
    }
}


function insertAndPairs(nums,start,end,value,sum){
    var j = 0
